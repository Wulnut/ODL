...
- Void: `void`
- Anything: `variant`
- File descriptor: `fd` (limited support in ambiroix backends)
...
> **NOTE**<br>
>
> When file descriptor uses as argument of RPC, File descriptor shouldn't close until backend send it to UDS. Generally backend uses file descriptor from RPC to make ancillary data and then close received file descriptors. So application **shouldn't** use/close file descriptors after put it to arguments for send via RPC input/Output argument. 

File descriptor as arguments require support handles from backend. 
* Backend should use UDS socket
* Backend should support send SCM_RIGHTS data via UDS socket [UNIX socket documentation](https://man7.org/linux/man-pages/man7/unix.7.html)
* backend should encode file descriptor especially 
* backend should close file descriptor after send it via UDS socket (depends on backend implementation: backend should close descriptors after sending, amxb_usp support parameters close-fd-after-send which control behavior and by default set to true)

At the moment of writing the following bus systems and protocols can be used for passing file descriptors over RPC:

- [pcb](https://gitlab.com/prpl-foundation/components/ambiorix/modules/amxb_backends/amxb_pcb) (SoftAtHome proprietary bus system). - as input/output arguments.
- [USP](https://gitlab.com/prpl-foundation/components/ambiorix/modules/amxb_backends/amxb_usp/-/blob/main/README.md#file-Descriptors-transferring) - only as output arguments
...
An event can't define list of arguments via ODL. On implementation Event can add arguments to body, but list of arguments unknown on datamodel.

The valid types for event arguments are:
...
Passing File descriptors as arguments require support from implementation of Ambiorix bus backend. more details: [RPC File descriptor argument](#file-descriptor-argument) 

At the moment of writing the following bus systems and protocols can be used for pass file descriptors over Event:

- [pcb](https://gitlab.com/prpl-foundation/components/ambiorix/modules/amxb_backends/amxb_pcb) (SoftAtHome proprietary bus system). 
- [USP](https://gitlab.com/prpl-foundation/components/ambiorix/modules/amxb_backends/amxb_usp/-/blob/main/README.md#file-Descriptors-transferring).
...
Adding and removing of the attributes must be grouped, so either first remove attributes and then add attributes or vice-versa.
...
Declaring a parameter synchronization is similar as declaring a synchronization context. The difference is that is starts with the keyword `parameter` followed by the source parameter name and the target parameter name separated by a direction operator. Parameter synchronization declarations can only be done in the synchronization context or in an object synchronization. 
...
In some cases values must be transformed or translated before being applied on the destination. A `translate` and `apply` action can be added to parameter and object synchronization.
...
- The synchronization must be named, add `as <NAME>` to the synchronization declaration.
...
- **Word**: A string without any space characters (space, tab, newline), can contain any character (except spaces) and doesn't need to be put between quotes (single or double).
...
The attribute `%unique` indicates that only one instance can exists with the same value. When the attribute is used in combination with `%key` it becomes a key parameter which is not part of a combined key.
...
To save configuration options use `amxo_parser_save_config`.

To save a data model (or part of a data model) use `amxo_parser_save_object`.

To save the confiuration and a data model (or part of a data model) use  `amxo_parser_save`.
...
When saving the data model, starting from an object in the hierachical data model tree, the parent objects must all be persistent and the name (or identifierrs) of each object will be added to the saved file. This will ensure that loading the saved file will reproduce
the same objects and instances. The details (parameters or other instances) of any parent object will not be saved when starting from
an object in the hierarchical tree.

All persistent parameters of an object will be saved, this includes the private parameters. To get the list of the persistent parameters the odl save functionality uses private access. Parameters with read handler actions can return a different value depending on the access level, the value stored in the file is the value that will be retrieved using protected access.

Example:

Assume this data model definition
```
%define {
    %persistent object ObjectWithHiddenValue {
        %peristent string HiddenValue = "Secret" {
            on action read call hide_value;
        }
        %persistent string Value = "Text";
        %private %persistent string HiddenPrivate = "Private Secret" {
            on action read call hide_value;
        }
    }
}
...
When saving the above data model it will create a save file containing this data:

```
%populate {
    object ObjectWithHiddenValue {
        parameter HiddenValue = "Secret";
        parameter Value = "Text";
        parameter HiddenPrivate = "Private Secret";
    }
}
```

When fetching the object parameters the following result will be returned when using public access:
(JSON format)
```
{
